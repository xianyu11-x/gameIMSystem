...
TFuture<void> gateServer::csDestroyChannel(const int socketFd, const std::string& message, std::string& response) {
    ...
    ssChannelMsgReq->set_msgtype(protocol::sschannelmsg::SSChannelMsgType::EN_DESTROY);
    ...
    channelRsp->set_msgtype(protocol::csmsg::CSChannelMsgType::EN_DESTROY);
    ...
    if (!ssChannelMsgRsp.issuccess()) {
        ...
        logger->error("Destroy channel failed, player name: {}, error: {}",
                      ssChannelMsgRsp.sendplayer().playername(),
                      ssChannelMsgRsp.errmsg());
    } else {
        logger->info("Destroy channel success, player name: {}, channel name: {}",
                     ssChannelMsgRsp.sendplayer().playername(),
