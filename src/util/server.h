...
class baseServer {
public:
    baseServer(TEPoll& poller, std::string address, int bufferSize)
        : serverPoller(poller)
    ...
    TVoidTask client_handler(TEPoll::TSocket socket);

    TVoidTask ReadPacket(TEPoll::TSocket& socket, std::queue<std::string>& sendPackets);

    TVoidTask WritePacket(TEPoll::TSocket& socket, std::queue<std::string>& sendPackets);
