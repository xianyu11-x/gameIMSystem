...
TFuture<void> gateServer::csPullChannelList(const int socketFd, const std::string& message, std::string& response) {
    ...
    ssChannelMsgReq->set_msgtype(protocol::sschannelmsg::SSChannelMsgType::EN_PULL);
    ...
    channelRsp->set_msgtype(protocol::csmsg::CSChannelMsgType::EN_PULL);
    ...
    if (!ssChannelMsgRsp.issuccess()) {
        ...
        logger->error("Pull channel list failed, player name: {}, error: {}",
                      ssChannelMsgRsp.sendplayer().playername(),
                      ssChannelMsgRsp.errmsg());
    } else {
        // 复制所有频道信息到响应
        for (int i = 0; i < ssChannelMsgRsp.channelinfo_size(); i++) {
            channelRsp->add_channelinfo()->CopyFrom(ssChannelMsgRsp.channelinfo(i));
        }
        logger->info("Pull channel list success, player name: {}, channel count: {}",
                     ssChannelMsgRsp.sendplayer().playername(),
                     ssChannelMsgRsp.channelinfo_size());
    }
