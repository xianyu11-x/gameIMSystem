...
TFuture<void> gateServer::csCreateChannel(const int socketFd, const std::string& message, std::string& response) {
    ...
    ssChannelMsgReq->set_msgtype(protocol::sschannelmsg::SSChannelMsgType::EN_CREATE);
    ...
    channelRsp->set_msgtype(protocol::csmsg::CSChannelMsgType::EN_CREATE);
    ...
    if (!ssChannelMsgRsp.issuccess()) {
        ...
        logger->error("Create channel failed, player name: {}, error: {}",
                      ssChannelMsgRsp.sendplayer().playername(),
                      ssChannelMsgRsp.errmsg());
    } else {
        // 复制频道信息到响应
        if (ssChannelMsgRsp.channelinfo_size() > 0) {
            channelRsp->add_channelinfo()->CopyFrom(ssChannelMsgRsp.channelinfo(0));
        }
        logger->info("Create channel success, player name: {}, channel name: {}",
                     ssChannelMsgRsp.sendplayer().playername(),
