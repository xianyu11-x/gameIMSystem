...
TFuture<void> gateServer::csJoinChannel(const int socketFd, const std::string& message, std::string& response) {
    ...
    ssChannelMsgReq->set_msgtype(protocol::sschannelmsg::SSChannelMsgType::EN_JOIN);
    ...
    channelRsp->set_msgtype(protocol::csmsg::CSChannelMsgType::EN_JOIN);
    ...
    if (!ssChannelMsgRsp.issuccess()) {
        ...
        logger->error("Join channel failed, player name: {}, error: {}",
        ...
        logger->info("Join channel success, player name: {}, channel name: {}",
                     ssChannelMsgRsp.sendplayer().playername(),
